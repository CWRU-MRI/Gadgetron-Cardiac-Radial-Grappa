#include "ImageArraySplitDFGadget.h"

namespace Gadgetron{

ImageArraySplitDFGadget::ImageArraySplitDFGadget()
{
gt_timer_.set_timing_in_destruction(false);
}


int ImageArraySplitDFGadget::process( GadgetContainerMessage<IsmrmrdImageArray>* m1)
{
     if (perform_timing.value()) { gt_timer_.start("ImageArraySplitDFGadget::process");}
	...
	GDEBUG("[X,Y,Z,CHA,N,S,LOC]: [%d,%d,%d,%d,%d,%d,%d]",X,Y,Z,CHA,N,S,LOC);
...
GADGET_FACTORY_DECLARE(ImageArraySplitDFGadget)
}

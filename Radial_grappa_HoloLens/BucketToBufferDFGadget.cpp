...
#include "BucketToBufferDFGadget.h"
#include "mri_core_data.h"
#include "hoNDArray_elemwise.h"
#include "hoNDArray_reductions.h"
namespace Gadgetron{

    BucketToBufferDFGadget::BucketToBufferDFGadget()
    {
  ...
  int BucketToBufferDFGadget ::process(GadgetContainerMessage<IsmrmrdAcquisitionBucket>* m1)
  {
  if (perform_timing.value()) { gt_timer_.start("BucketToBufferDFGadget::process"); }

    size_t key;
    std::map<size_t, GadgetContainerMessage<IsmrmrdReconData>* > recon_data_buffers;

    //GDEBUG("BucketToBufferDFGadget::process\n");
  ...
  int BucketToBufferDFGadget::close(unsigned long flags)
  {

    int ret = Gadget::close(flags);
    GDEBUG("BucketToBufferDFGadget::close\n");

    return ret;
  }

  size_t BucketToBufferDFGadget::getSlice(ISMRMRD::ISMRMRD_EncodingCounters idx)
  {
    size_t index;
  ...
  size_t BucketToBufferDFGadget::getN(ISMRMRD::ISMRMRD_EncodingCounters idx)
  {
    size_t index;
  ...
  size_t BucketToBufferDFGadget::getS(ISMRMRD::ISMRMRD_EncodingCounters idx)
  {
    size_t index;
  ...
  size_t BucketToBufferDFGadget::getKey(ISMRMRD::ISMRMRD_EncodingCounters idx)
  {
  ...
  IsmrmrdReconBit & BucketToBufferDFGadget::getRBit(std::map<size_t, GadgetContainerMessage<IsmrmrdReconData>* > & recon_data_buffers, size_t key, uint16_t espace)
  {
  ...
  void BucketToBufferDFGadget::allocateDataArrays(IsmrmrdDataBuffered & dataBuffer, ISMRMRD::AcquisitionHeader & acqhdr, ISMRMRD::Encoding encoding, IsmrmrdAcquisitionBucketStats & stats, bool forref)
  {
  ...
  void BucketToBufferDFGadget::fillSamplingDescription(SamplingDescription & sampling, ISMRMRD::Encoding & encoding, IsmrmrdAcquisitionBucketStats & stats, ISMRMRD::AcquisitionHeader& acqhdr, bool forref)
  {
  ...
  void BucketToBufferDFGadget::stuff(std::vector<IsmrmrdAcquisitionData>::iterator it, IsmrmrdDataBuffered & dataBuffer, ISMRMRD::Encoding encoding, IsmrmrdAcquisitionBucketStats & stats, bool forref)
  {
  ...
  GADGET_FACTORY_DECLARE(BucketToBufferDFGadget)

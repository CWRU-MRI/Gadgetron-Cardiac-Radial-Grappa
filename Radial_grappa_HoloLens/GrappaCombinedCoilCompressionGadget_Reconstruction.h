/** \file   GrappaCombinedCoilCompressionGadget_Reconstruction.h
...
Modified 10/30/2016 by DF from GenericReconEigenChannelGadget
*/
...
namespace Gadgetron {
    ...
    class EXPORTGADGETSMRICORE GrappaCombinedCoilCompressionGadget_Reconstruction : public GenericReconDataBase
    {
    public:
        GADGET_DECLARE(GrappaCombinedCoilCompressionGadget_Reconstruction);

        typedef GenericReconDataBase BaseClass;
        typedef hoNDKLT< std::complex<float> > KLTType;

        GrappaCombinedCoilCompressionGadget_Reconstruction();
        ~GrappaCombinedCoilCompressionGadget_Reconstruction();
	...
	//DNF added
	hoNDArray<std::complex<float>> & imported_KL_coeff_;
	void apply_eigen_channel_coefficients_df(const std::vector< std::vector< std::vector< hoNDKLT<std::complex<float> > > > >& KLT, hoNDArray<std::complex<float> >& data, int);
	int total_slices_;
    };
}
